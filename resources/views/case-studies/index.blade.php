        ...
        <div class="container-veenso">
            <div class="reveal relative overflow-hidden rounded-[2rem] border border-veenso-border bg-gradient-to-br from-veenso-elevated via-veenso-charcoal to-veenso-bg px-8 py-16 text-center lg:px-16">
                ...
                <div class="relative z-10 flex flex-col items-center gap-6">
                    <h2 class="font-display max-w-xl text-2xl font-bold leading-snug text-veenso-text sm:text-3xl">{{ $siteSettings['cta_case_title'] ?? 'Want results like these?' }}</h2>
                    <p class="max-w-xl text-veenso-muted">{{ $siteSettings['cta_case_text'] ?? "Tell us where you want to be in 12 months — we'll show you the strategy to get there." }}</p>
                    <x-button :href="route('contact')" variant="primary" glow>{{ $siteSettings['cta_case_button'] ?? 'Book a Strategy Call' }}</x-button>
                </div>
            </div>
        </div>
    </section>
