...
@section('content')
    ...
    @if ($secondaryServices->isNotEmpty())
        <section class="section-y bg-veenso-charcoal/40">
            <div class="container-veenso section-stack">
                <x-section-heading eyebrow="Specialized Services" title="Additional capabilities that extend your growth stack" align="left" class="mx-0" />
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($secondaryServices as $index => $service)
                        <x-service-card :service="$service" :index="$index" />
                    @endforeach
                </div>
            </div>
        ...
        <div class="container-veenso">
            <div class="reveal relative overflow-hidden rounded-[2rem] border border-veenso-border bg-gradient-to-br from-veenso-elevated via-veenso-charcoal to-veenso-bg px-8 py-16 text-center lg:px-16">
                ...
                <div class="relative z-10 flex flex-col items-center gap-6">
                    <h2 class="font-display max-w-xl text-2xl font-bold leading-snug text-veenso-text sm:text-3xl">{{ $siteSettings['cta_services_title'] }}</h2>
                    <p class="max-w-xl text-veenso-muted">{{ $siteSettings['cta_services_text'] }}</p>
                    <x-button :href="route('contact')" variant="primary" glow>{{ $siteSettings['cta_services_button'] }}</x-button>
                </div>
            </div>
        </div>
    </section>
